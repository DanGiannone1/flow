...
def _resource_attributes() -> dict[str, str]:
    ...
    attrs = {
        "service.name": os.getenv("OTEL_SERVICE_NAME", "tax-workbench-session"),
        "service.namespace": os.getenv("OTEL_SERVICE_NAMESPACE", "tax-workbench"),
    }
...
def setup_tracing(app: Optional[FastAPI] = None) -> None:
    ...
    try:
        ...
        _tracer = trace.get_tracer("tax-workbench.session-container")
        _enabled = True
        logger.info(
